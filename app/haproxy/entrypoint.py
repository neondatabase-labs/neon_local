...
def _prepare_config() -> None:
    ...
    with open("/scripts/haproxy.cfg.tmpl", "r") as file:
...
def _start_haproxy() -> subprocess.Popen:
    _prepare_config()
    return subprocess.Popen(["haproxy", "-f", "/tmp/haproxy.cfg"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)


def _haproxy_thread(cv: threading.Condition) -> None:
    haproxy_process = _start_haproxy()
...
def _inner_main() -> None:
    ...
    haproxy_thread = threading.Thread(target=_haproxy_thread, args=(cv,))
    haproxy_thread.start()
