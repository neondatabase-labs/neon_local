...
class JavaScriptStressTest {
  ...
  async runSingleTest(runNumber) {
    return new Promise((resolve) => {
      ...
      const testProcess = spawn('node', ['run-javascript-tests.js'], {
        cwd: __dirname,
  ...
  generateReport() {
    ...
    if (runsWith503 > 0) {
      console.log(`\n🚨 WARNING: ${runsWith503} runs encountered 503 errors!`);
      console.log('   This suggests potential proxy or connection issues.');
    }
    
    if (runsWithRetries > 0) {
      console.log(`\n🔄 INFO: ${runsWithRetries} runs attempted retries.`);
...
// Run the stress test
const stressTest = new JavaScriptStressTest();
